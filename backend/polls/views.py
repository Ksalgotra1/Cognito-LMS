...
def vote(request, question_id):
    ...
    try:
        ...
        # Use F() expression to avoid race conditions when multiple votes occur.
        selected_choice.votes = F("votes") + 1
        selected_choice.save()
        # Reload object to resolve F() to concrete value if needed later:
...
def health_check(request):
    return JsonResponse({'status': 'ok', 'message': 'Django is alive!'})
