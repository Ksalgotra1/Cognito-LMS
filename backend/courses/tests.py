...
class BaseTestMixin:
    ...
    def setUp(self):
        self.student = User.objects.create_user(
            username="student1", email="student@test.com", password="testpass123", role=User.Role.STUDENT
        )
        self.instructor = User.objects.create_user(
            username="instructor1", email="instructor@test.com", password="testpass123", role=User.Role.INSTRUCTOR
        )
        self.course = Course.objects.create(title="Data Structures", description="Learn DS", instructor=self.instructor)
        self.module = Module.objects.create(course=self.course, title="Arrays", order=1)
...
class CourseModelTest(BaseTestMixin, TestCase):
    ...
    def test_direct_cycle_detected(self):
        ...
        course_b = Course.objects.create(title="Algorithms", description="Learn Algo", instructor=self.instructor)
...
class UserModelTest(TestCase):
    ...
    def test_default_role_is_student(self):
        ...
        user = User.objects.create_user(username="newuser", email="new@test.com", password="pass123")
...
class CourseAPITest(BaseTestMixin, TestCase):
    ...
    def test_create_course_authenticated(self):
        ...
        response = self.client.post("/api/courses/", {"title": "New Course", "description": "A new course"})
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Course.objects.count(), 2)

    def test_create_course_unauthenticated_rejected(self):
        """Unauthenticated users cannot create courses."""
        response = self.client.post("/api/courses/", {"title": "Hacker Course", "description": "Should fail"})
...
class QuizAPITest(BaseTestMixin, TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.question = Question.objects.create(lesson=self.lesson, text="What is an array?")
        self.correct = Choice.objects.create(question=self.question, text="A collection of elements", is_correct=True)
        self.wrong = Choice.objects.create(question=self.question, text="A single value", is_correct=False)
        self.client.force_authenticate(user=self.student)
    ...
    def test_submit_quiz_grading(self):
        ...
        # Submit wrong answer
        response = self.client.post(
            f"/api/courses/lessons/{self.lesson.id}/quiz/submit/", {str(self.question.id): self.wrong.id}, format="json"
        )
...
class ProfileAPITest(BaseTestMixin, TestCase):
    ...
    def test_update_profile(self):
        ...
        response = self.client.patch("/api/courses/profile/", {"first_name": "John", "last_name": "Doe"}, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class CertificateVerifyTest(BaseTestMixin, TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.cert = Certificate.objects.create(user=self.student, course=self.course)

    def test_verify_valid_certificate(self):
        """Valid UUID returns certificate details."""
        response = self.client.get(f"/api/courses/certificate/verify/{self.cert.certificate_id}/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_verify_invalid_certificate(self):
        ...
        response = self.client.get(f"/api/courses/certificate/verify/{fake_uuid}/")
...
class DashboardAPITest(BaseTestMixin, TestCase):
    ...
    def test_dashboard_progress_updates(self):
        ...
        UserProgress.objects.create(user=self.student, lesson=self.lesson, is_completed=True)
        response = self.client.get("/api/courses/dashboard/stats/")
...
class CourseOwnershipSecurityTest(BaseTestMixin, TestCase):
    ...
    def test_non_owner_cannot_update_course(self):
        ...
        response = self.client.put(
            f"/api/courses/{self.course.id}/", {"title": "Hacked Title", "description": "Hacked"}, format="json"
        )
    ...
    def test_non_owner_cannot_patch_course(self):
        ...
        self.client.force_authenticate(user=self.student)
        response = self.client.patch(f"/api/courses/{self.course.id}/", {"title": "Sneaky Patch"}, format="json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_owner_can_update_course(self):
        ...
        self.client.force_authenticate(user=self.instructor)
        response = self.client.patch(f"/api/courses/{self.course.id}/", {"title": "Updated by Owner"}, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class RegistrationRoleSecurityTest(TestCase):
    ...
    def test_cannot_register_as_admin(self):
        ...
        response = self.client.post(
            "/api/register/",
            {"username": "hacker_admin", "email": "hacker@test.com", "password": "hackpass123", "role": "ADMIN"},
            format="json",
        )
    ...
    def test_cannot_register_as_instructor(self):
        ...
        response = self.client.post(
            "/api/register/",
            {"username": "fake_instructor", "email": "fake@test.com", "password": "hackpass123", "role": "INSTRUCTOR"},
            format="json",
        )
        self.assertIn(response.status_code, [status.HTTP_200_OK, status.HTTP_201_CREATED])
