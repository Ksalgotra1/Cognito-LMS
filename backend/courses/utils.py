...
class TrieNode:
    ...
    __slots__ = ["children", "data", "is_end_of_word"]
...
def generate_study_schedule(start_date, target_date, weekly_availability, lessons):
    ...
    while lesson_idx < total_lessons:
        if current_date > target_date:
            raise ValueError(
                f"Cannot fit all lessons by {target_date.strftime('%b %d, %Y')}. Please extend your target date or increase your daily availability."
            )
        ...
        daily_plan = {"date": current_date.isoformat(), "day": day_name, "lessons": []}
        ...
        while lesson_idx < total_lessons:
            ...
            if time_used_today + duration <= minutes_available:
                daily_plan["lessons"].append({"title": lesson.title, "duration": duration, "id": lesson.id})
